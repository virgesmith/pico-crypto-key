...
namespace
{
  ...
  ErrorMapper error(ErrorMapper::EC, {MBEDTLS_ERR_ECP_BAD_INPUT_DATA,
...
bytes ecdsa::pubkey(const mbedtls_ecp_keypair &ec_key)
{
  ...
  error.check(mbedtls_ecp_point_write_binary(&ec_key.grp,
                                             &ec_key.Q,
...
bytes ecdsa::sign(const mbedtls_ecp_keypair &key, const bytes &hash)
{
  ...
  error.check(mbedtls_ecdsa_sign_det_ext(const_cast<mbedtls_ecp_group *>(&key.grp) /*?*/, &r, &s, &key.d,
